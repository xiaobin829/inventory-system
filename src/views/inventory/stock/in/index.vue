    ...
    >
      ...
      >
        <t-row :gutter="16">
          ...
          <t-col :span="6">
            <t-form-item label="入库仓库" name="warehouseId">
              <t-select
          ...
          <t-col :span="6">
            <t-form-item label="供应商" name="supplierId">
              <t-select
                ...
                placeholder="请选择供应商"
                clearable
                :disabled="dialogMode === 'view' || !showSupplier"
              >
                <t-option
        ...
        <!-- 物料明细表格 -->
        <t-form-item label="入库明细" name="items" class="item-form-item">
...
import type { InboundBill, InboundBillQuery, BillItem } from '@/types/bill';
import { InboundType } from '@/types/bill';
...
// 是否显示供应商选择（采购入库/退货入库时显示）
const showSupplier = computed(() => {
  return formData.type === InboundType.PURCHASE || formData.type === InboundType.RETURN;
});
...
const loadOptions = async () => {
  try {
    // 并行加载所有选项数据
    const [warehouseRes, suppliers, materialRes, products] = await Promise.all([
...
const handleSubmit = async () => {
  ...
  if (formData.items.length === 0) {
    MessagePlugin.warning('请至少添加一项入库物料');
    return;
  }
...
const handleExport = () => {
  ...
  // 构建CSV内容
  const headers = ['单据编号', '入库类型', '入库仓库', '供应商', '总数量', '总金额', '状态', '操作人', '创建时间'];
  ...
  link.setAttribute('download', `入库单_${new Date().toISOString().slice(0, 10)}.csv`);
